...
def getNegRatings(ratingList, itemTotal, all_dicts=None, n_samp=None):
    ni = []
    neg_set = set()
    
    #print(all_dicts)
    #print(len(all_dicts[0]))
    #print(len(ratingList))
    #print(ratingList[0])
    #print(all_dicts[0][0])
    # ratingList = [(12,24)]
    # itemTotal = 220
    # all_dicts = [{12:{24,13,2}}, {14:{12,13}}]
    # print(all_dicts[0])
    for rating in ratingList:
        c_u = rating[0]
        oldItem = rating[1]
        # rating exists
        fliter_items = None  # oscilante
        ...
        if all_dicts is not None:
            ...
            for dic in all_dicts:
                if c_u in dic:
                    ...
                    fliter_items.update(dic[c_u])  # fliter items is a dictionary!

        #print('--------------------- THIS IS OUR RATING:')
        #print(c_u, oldItem)
        #while True:
        while len(neg_set) < n_samp: # diana
            newItem = random.randrange(itemTotal)
            #print(itemTotal)
            #print(newItem)
            # print('--------------------- THIS IS THE NEG SET:')
            # print(neg_set)
            # print('--------------------- THESE ARE THE FLITER ITEMS:')
            # print(fliter_items)
            ...
            if newItem != oldItem and newItem not in fliter_items and newItem not in neg_set:
                break
            # else:
            #     print('I AM INNNNNNNNNNNNNNNNNNNNN')
            # elif len(neg_set) == itemTotal:
            #     #print('wrong break')
            #     break
            # else:
            #     #print('stuck')
            #     break
        # if 1 < 2:
        #     print('SOMETIMES I SHALL PASS!!!')
        #     break
        ni.append(newItem)
        neg_set.add(newItem)
        #print('I AM OUTTTTTTTTTTTTTTTTTT')
        #if len(neg_set) == 370:
            #break
        #print(len(neg_set))
